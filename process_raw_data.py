...
def get_opponent_dst_stats_for_previous_weeks(opponent, week, year, df_dst):
  ...
  return round(mean(ypa[-3:]), 1), round(mean(rtd[-3:]), 1)
...
if __name__ == '__main__':
  ...
  columns_to_drop = ['del', 'first', 'last', 'pa', 'pc', 'py', 'ints', 'tdp', 'ret', 'rety', 'tdret', 'peny', 'conv',
                     'fp2', 'fp3']
    ...
    for year in player_rows.year.unique():
      ...
      for i in range(3, len(year_rows)):
        ...
        for stat in stats_to_average:
          row[stat] = round(mean(year_rows[i - 3:i][stat]), 1)
